...
class Config:
    # ── 데이터
    data_dir: str = 'DataSet/Processed/1_/train'
    in_size: tuple = (256, 256)    # [추정] 잠재 8×8 → 5단 다운샘플 역산. 정사각이라 rot90도 안전
    ...
    run_bt: bool = True
    bt_ckpt: str = 'BT_pretrained_1.pth'
...
class BT_VAE(nn.Module):
    def __init__(self, z_dim=1024, stride=2, kernel_size=3, padding=1,
        ...
        # 256 → 128 → 64 → 32 → 16 → 8 (stride-2 conv 5단)
        self.encoder = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size, stride, padding),
            nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(64, 1, kernel_size, stride, padding, output_padding),
...
if __name__ == '__main__':
    ...
    op2 = torch.optim.AdamW(model.parameters(), lr=Config.lr2,
                            weight_decay=Config.weight_decay)
    best_dice = 0.0
    tag = f"bp_r{int(Config.label_ratio*100)}{'_bt' if Config.run_bt else ''}"
